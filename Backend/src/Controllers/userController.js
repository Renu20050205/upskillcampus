...
const updateUser = async (req, res) => {
    try {
        ...
        if (userId !== loggedInUserId) {
            return res.status(403).json({ msg: "Bad Authorisatiion!!! Invalid Token!" })
        }
        ...
        let update = await userModel.findByIdAndUpdate(userId,
            { name, email, contact, password: hashedPassword, address, gender, age }, { new: true });
...
const loginUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: user._id, email: user.email }, "my-secret-key", { expiresIn: "1h" });
        return res.status(200).json({ msg: "Longin Succesfull", token });
